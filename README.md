...
4. **Recommends the optimal strike** — picks call vs. put, then chooses ITM / ATM /
...
5. **Prices the contract** — Black-Scholes premium, delta, gamma, theta/day,
   breakeven, and implied vol for the remaining session, plus an underlying
   entry / target / stop plan with risk:reward and an invalidation level.
...
| `lib/spy-agent/market-data.ts` | Live data fetch + deterministic simulated fallback |
| `lib/spy-agent/strike-engine.ts` | Signal scoring, conviction, Black-Scholes, strike selection |
| `lib/spy-agent/agent.ts` | Orchestrator → assembles the full report + reasoning |
| `app/api/spy-agent/route.ts` | `GET /api/spy-agent?instrument=SPY\|SPXW` JSON endpoint |
| `app/spy-agent/page.tsx` | Live dashboard UI (bias gauge, signals, greeks, price chart) |
